...
class QARReader():
    ...
    def is_flight(self):
        ...
        if flight == monstr:
            ...
            if check:
                ...
                self.flights.append(self.index)
    ...
    def get_flight_intervals(self):
        i = 0
        for each in self.flights:
            try:
                self.flight_intervals.append((self.flights[i], self.flights[i + 1]))
            except IndexError:
                self.flight_intervals.append((self.flights[i],
                                              self.get_last_flight_end(self.flights[i])))
            i += 1
    ...
    def get_durations(self):
        ...
        for each in self.headers:
            ...
            end = self.curr_date[i] + datetime.timedelta(seconds=duration_in_sec)
    ...
    def get_initial_date(self, header):
        ...
        #self.init_counter = int(((hex(ord(header[109]))) + (hex(ord(header[108])))[2:] +
                                #(hex(ord(header[107])))[2:] + (hex(ord(header[106])))[2:]), 16)  # add to str 4 bytes in hex \
    ...
    def get_current_date(self, header):
        ...
        if curr_counter < self.init_counter:
            ...
            diff = (curr_counter - self.init_counter)/256
        #self.curr_date.append((curr_counter, diff))
        self.curr_date.append((self.init_date + datetime.timedelta(seconds=diff)))
